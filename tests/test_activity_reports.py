"""
tests/ew_track/test_activity_reports.py
...
def activity_report(client):
    resp = client.post(BASE, json={"submitted_by": "operator1", "classification": CLS})
...
class TestCreateReport:
    ...
    def test_create_missing_submitted_by_returns_422(self, client):
        assert client.post(BASE, json={"classification": CLS}).status_code == 422

    def test_create_missing_classification_returns_422(self, client):
        assert client.post(BASE, json={"submitted_by": "op1"}).status_code == 422

    def test_create_invalid_classification_returns_422(self, client):
        assert client.post(BASE, json=_report_payload(classification="ULTRA_SECRET")).status_code == 422
...
class TestUpdateReport:
    ...
    def test_patch_name(self, client, activity_report):
        resp = client.patch(f"{BASE}/{activity_report['id']}", json={"name": "Updated Report"})
        assert resp.status_code == 200
        assert resp.json()["name"] == "Updated Report"

    def test_patch_classification(self, client, activity_report):
        resp = client.patch(f"{BASE}/{activity_report['id']}", json={"classification": "UNCLASSIFIED"})
        assert resp.status_code == 200
    ...
    def test_patch_not_found_returns_404(self, client):
        assert client.patch(f"{BASE}/{MISSING_UUID}", json={"name": "X"}).status_code == 404
...
class TestDeleteReport:
    ...
    def test_delete_cascades_instances(self, client, activity_report):
        client.post(f"{BASE}/{activity_report['id']}/instances",
...
class TestInstances:
    ...
    def test_add_instance_returns_201(self, client, activity_report):
        resp = client.post(f"{BASE}/{activity_report['id']}/instances",
                           json=_instance_payload(activity_report["id"]))
        assert resp.status_code == 201
        data = resp.json()
    ...
    def test_add_instance_with_all_fields(self, client, activity_report):
        resp = client.post(f"{BASE}/{activity_report['id']}/instances", json=_instance_payload(
            activity_report["id"],
    ...
    def test_duplicate_instance_for_same_track_returns_409(self, client, activity_report, ew_track):
        ...
        assert client.post(f"{BASE}/{activity_report['id']}/instances", json=payload).status_code == 409

    def test_last_seen_before_first_seen_returns_422(self, client, activity_report):
        resp = client.post(f"{BASE}/{activity_report['id']}/instances",
                           json=_instance_payload(activity_report["id"],
    ...
    def test_invalid_lat_returns_422(self, client, activity_report):
        assert client.post(f"{BASE}/{activity_report['id']}/instances",
                           json=_instance_payload(activity_report["id"],
                               last_position_latitude_dd=95.0)).status_code == 422

    def test_invalid_lon_returns_422(self, client, activity_report):
        assert client.post(f"{BASE}/{activity_report['id']}/instances",
                           json=_instance_payload(activity_report["id"],
                               last_position_longitude_dd=190.0)).status_code == 422

    def test_list_instances_returns_all(self, client, activity_report):
        for i in range(3):
            client.post(f"{BASE}/{activity_report['id']}/instances", json=_instance_payload(
                activity_report["id"],
                last_position_latitude_dd=24.0 + i * 0.1,
                first_seen_dtg=f"2024-06-0{i+1}T10:00:00Z",
                last_seen_dtg=f"2024-06-0{i+1}T11:00:00Z",
            ))
        assert len(client.get(f"{BASE}/{activity_report['id']}/instances").json()) == 3

    def test_get_single_instance(self, client, activity_report):
        instance = client.post(f"{BASE}/{activity_report['id']}/instances",
                               json=_instance_payload(activity_report["id"])).json()
    ...
    def test_get_instance_wrong_report_returns_404(self, client, activity_report):
        instance = client.post(f"{BASE}/{activity_report['id']}/instances",
                               json=_instance_payload(activity_report["id"])).json()
        assert client.get(f"{BASE}/{MISSING_UUID}/instances/{instance['id']}").status_code == 404
...
class TestMissionLinks:
    ...
    def test_list_linked_missions(self, client, activity_report, mission):
        client.post(f"{BASE}/{activity_report['id']}/missions/{mission['id']}")
        mission_ids = [m["mission_id"] for m in
                       client.get(f"{BASE}/{activity_report['id']}/missions").json()]
    ...
    def test_list_missions_empty_for_new_report(self, client, activity_report):
        assert client.get(f"{BASE}/{activity_report['id']}/missions").json() == []